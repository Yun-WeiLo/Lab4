...
def main():
    ...
    # --- Plot 2: Verified Trajectory 3D (Correct Shape) ---
    print("Verifying FK and Plotting 3D...")
    ...
    ax1.plot(actual_p_trace[0, :], actual_p_trace[1, :], actual_p_trace[2, :], 'b-', linewidth=2, label='p(t)')
    ax1.scatter(actual_p_trace[0, 0], actual_p_trace[1, 0], actual_p_trace[2, 0], c='g', marker='o', s=60, label='start', depthshade=False)
    ax1.scatter(actual_p_trace[0, -1], actual_p_trace[1, -1], actual_p_trace[2, -1], c='r', marker='x', s=60, label='end', depthshade=False)
    ax1.set_title('Verified Trajectory in s frame', fontsize=14, y=1.02)
    ax1.set_xlabel('x (m)'); ax1.set_ylabel('y (m)'); ax1.set_zlabel('z (m)')
    
    # 【關鍵修復】使用實際數據比例，不讓 Matplotlib 自動拉成正方體
    X = actual_p_trace[0, :]; Y = actual_p_trace[1, :]; Z = actual_p_trace[2, :]
    ax1.set_box_aspect((np.ptp(X), np.ptp(Y), np.ptp(Z)))
    ax1.view_init(elev=20, azim=-45)
    ax1.legend(loc='upper right')
    
    # --- Plot 3: Manipulability (Correct Calculation) ---
    print("Calculating Manipulability...")
    ...
    plt.plot(t, det_Jb * 1000.0, color='#0072BD', linewidth=1.5)
    plt.title('Manipulability', fontweight='bold', fontsize=12)
    plt.xlabel('time (s)', fontsize=10)
    plt.ylabel(r'det of $J_B$', fontsize=10)
    plt.text(0, 1.02, r'$\times 10^{-3}$', transform=plt.gca().transAxes, fontsize=10)
    ...
    # --- Save CSV ---
    print("Saving CSV...")
    led_col = np.ones(len(t))
    data_to_save = np.column_stack((t, theta_trajectory.T, led_col))
    username = "lo156" 
    filename = f"{username}.csv"
    np.savetxt(filename, data_to_save, delimiter=',', fmt='%.6f')
